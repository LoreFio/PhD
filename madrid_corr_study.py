...
def avg_corr(discriminant_column):
    """
    ...
    """
    df_file_list = [file for file in os.listdir(madrid_corr_dir) if ".pkl" in file and discriminant_column in file]
    first_df = pd.read_pickle(os.path.join(madrid_corr_dir, df_file_list[0]))
    ...
    for file in df_file_list:
        df = pd.read_pickle(os.path.join(madrid_corr_dir, file))
...
if __name__ == "__main__":
    print(avg_corr(discriminant_column=station_col))
    print(avg_corr(discriminant_column=pollutant_col))
