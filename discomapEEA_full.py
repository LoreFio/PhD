...
from discomapEEA_utilities import get_stat_data_name, download_to_path, pollutant_dict_eea, URL_COL, EEA_data_dir
...
if __name__ == "__main__":
    ...
    for p_number, p_name in pollutant_dict_eea.items():
        ...
        tmp_file = os.path.join(EEA_data_dir, "tmp.csv")
        ...
        if len(sub_url_data) > 0:
            output_dir = os.path.join(EEA_data_dir, f"{CountryCode}_{CityName[:3]}_{Year_from}_{Year_to}_{p_name}")
            os.mkdir(output_dir)
